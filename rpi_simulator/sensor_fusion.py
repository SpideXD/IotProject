"""
Sensor Fusion for RPi Simulator.

Provides weighted fusion of camera and radar data with agreement bonus.
...
logger = logging.getLogger("rpi_simulator.sensor_fusion")

@dataclass
class CameraResult:
    """Result from camera detection."""
    object_type: str = "empty"
    confidence: float = 0.0

@dataclass
class RadarResult:
    """Result from radar sensor."""
    presence: float = 0.0
    motion: float = 0.0
...
class SensorFusion:
    """
    Sensor fusion using weighted combination of camera and radar.

    Camera weight: 60%
    Radar weight: 40%
    Agreement bonus: 10% when both sensors agree
    """
    ...
    def fuse(
        ...
        """
        Fuse camera and radar results into a single occupancy assessment.

        Args:
            camera_result: Camera detection result
            radar_result: Radar sensor result

        Returns:
            FusedResult with occupancy score and presence detection
        """
        cam_conf = 0.0
        cam_type = "empty"
        radar_pres = 0.0
        ...
        # Agreement bonus when both sensors agree on presence

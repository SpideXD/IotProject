...
class GhostDetector:
    ...
    def __init__(
        ...
        self._seat_to_zone: Dict[str, str] = {}

    def set_seat_zones(self, seat_to_zone: Dict[str, str]):
        """Set the seat to zone mapping."""
        self._seat_to_zone = seat_to_zone
    ...
    def _make_alert(
        ...
        zone_id = self._seat_to_zone.get(seat_id, "unknown")
    ...
    def get_all_seats(self) -> Dict[str, dict]:
        """Get state for all seats."""
        result = {}
        for seat_id, rec in self._seats.items():
            result[seat_id] = {
                "state": rec.state.value,
                "object_type": rec.last_object_type,
                "is_occupied": rec.state in (SeatState.OCCUPIED, SeatState.SUSPECTED_GHOST, SeatState.CONFIRMED_GHOST),
                "dwell_time": round(time.time() - rec.state_entered_time, 1),
                "time_since_motion": round(time.time() - rec.last_motion_time, 1),
            }
        return result

    def save_state(self, filepath: str):
        """Save FSM state to JSON file for persistence across restarts."""
        import json
        data = {}
        for seat_id, rec in self._seats.items():
            data[seat_id] = {
        ...
        with open(filepath, 'w') as f:
            json.dump(data, f)
        logger.info(f"FSM state saved to {filepath} ({len(data)} seats)")

    def load_state(self, filepath: str):
        """Load FSM state from JSON file. Called on startup to restore state."""
        import json
        if not os.path.exists(filepath):
            logger.info(f"No saved FSM state found at {filepath}")
            return
        try:
            with open(filepath, 'r') as f:
                data = json.load(f)
            for seat_id, rec_data in data.items():
                self._seats[seat_id] = SeatRecord(
                    state=SeatState(rec_data["state"]),
                    last_motion_time=rec_data["last_motion_time"],
                    state_entered_time=rec_data["state_entered_time"],
                    last_update_time=rec_data["last_update_time"],
                    last_object_type=rec_data["last_object_type"],
                    last_occupancy_score=rec_data["last_occupancy_score"],
                )
            logger.info(f"FSM state loaded from {filepath} ({len(data)} seats)")
